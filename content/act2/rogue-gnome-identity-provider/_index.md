...
Logging into the terminal and reading the welcome text: 
...
We have now obtained JWT token, now authenticating to `gnome-48371.atnascorp``:
...
By this I have obtained a session cookie and can pass that on:
...
From looking at the `http://idp.atnascorp/.well-known/jwks.json` file I assume that the server is using this to validate the tokens. My plan is to fool the server to reach out to my variant, and see if I can fool my way in! 
...
Creating a new JWT. Here I change `admin` to `true`, and instruct the server to use `http://paulweb.neighborhood/jwttool_custom_jwks.json` as `jku`:
```bash
